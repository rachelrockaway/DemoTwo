...
//  DemoTwo
//
//  Created by Rachel Stevenson on 6/19/15.
...
//        case ConfirmButton = "CONFIRMBUTTON"
    ...
    let kStandardBackgroundColor = UIColor.whiteColor()
...
*/
    ...
    //            let searchProfile = UIStoryboard(name:"Main", bundle: nil).instantiateViewControllerWithIdentifier("VCQ") as? UIViewController
    ////
    //
    //            let transition = CATransition()
    //            transition.duration = 0.4
    ...
    func saveData(){
        ...
        if let fullName = form.formRowWithTag(Tags.WaveHeight.rawValue).value as? String {
            results[Tags.WaveHeight.rawValue] = fullName
            println(results)
        }
